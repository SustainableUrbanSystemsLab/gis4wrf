...
def get_namelist_schema(name: str) -> dict:
    if name not in SCHEMA_CACHE:
        ...
        if not os.path.exists(schema_path):
            # Fall back to YAML source if JSON hasn't been generated by build.py
            yaml_path = os.path.join(SCHEMA_DIR, name + '.yml')
            if os.path.exists(yaml_path):
                import yaml
                with open(yaml_path, encoding='utf-8') as f:
                    schema = yaml.safe_load(f)
                # Cache the JSON for next time
                with open(schema_path, 'w', encoding='utf-8') as f:
                    json.dump(schema, f, indent=1)
            else:
                raise FileNotFoundError(
                    f'Namelist schema not found: {schema_path} (or {yaml_path}). '
                    'Run build.py to generate schema files.')
        else:
            ...
            for _, var_val in group.items():
                if 'options' in var_val:
                    ...
                    if var_val['type'] == 'list':
                        ...
                        t = var_val['type']
                    if t == 'int':
                        parsefn = lambda k: int(k) if isinstance(k, str) else int(k)
                    elif t == 'bool':
                        parsefn = lambda k: k if isinstance(k, bool) else k.lower() == 'true'
                    else:
                        continue
                    var_val['options'] = {
                        parsefn(k): v
